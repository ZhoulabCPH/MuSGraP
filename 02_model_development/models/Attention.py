...
class Attention_Gated(nn.Module):
    def __init__(self, L=512, D=128, K=1, num_heads=8):
        ...
        # 多头注意力：我们需要将 attention_V 和 attention_U 分成多个头
        self.attention_V_heads = nn.ModuleList([self.attention_V for _ in range(self.num_heads)])
        self.attention_U_heads = nn.ModuleList([self.attention_U for _ in range(self.num_heads)])

        # 每个头的注意力权重矩阵
        self.attention_weights_heads = nn.ModuleList([self.attention_weights for _ in range(self.num_heads)])

    def forward(self, x, isNorm=True):
        A_all_heads = []  # 用于保存每个头的注意力权重

        # 对每个头分别计算注意力
        for i in range(self.num_heads):
            A_V = self.attention_V_heads[i](x)  # NxD
            A_U = self.attention_U_heads[i](x)  # NxD
            A = self.attention_weights_heads[i](A_V * A_U)  # NxK
            A = torch.transpose(A, 1, 0)  # KxN

            # 保存每个头的注意力矩阵
            A_all_heads.append(A)

        # 将所有头的注意力矩阵拼接起来
        A = torch.cat(A_all_heads, dim=0)  # [num_heads*K, N]

        Patch_Weight = A[:, 0]  # 提取第一个维度作为样本的注意力权重

        if isNorm:
            A = F.softmax(A, dim=1)  # 对注意力分数进行归一化，softmax over N

        return A, Patch_Weight  # 返回多头注意力矩阵和单个头的注意力权重
